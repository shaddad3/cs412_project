...
import { datasetPresets } from './presetConfigs';
...
function makeGUI() {
  ...
  if (state.hideText) {
    ...
    // Add preset config to UI when utofill & Explain button is clicked
    d3.select("#autofill-button").on("click", () => {
      const datasetKey = getKeyFromValue(datasets, state.dataset);
      const preset = datasetPresets[datasetKey];
  
      if (!preset) { // alert message if we do not have a preset config for the dataset
        alert("No preset available for this dataset.");
        return;
      }
  
      // Apply preset configuration to the playground state
      state.networkShape = preset.layers; // set number of layers
      state.numHiddenLayers = preset.layers.length; // set number of units per layer
      // Set activation function and update UI
      state.activation = activations[preset.activation];
      d3.select("#activations").property("value", preset.activation);
      // Set learning rate and update UI
      state.learningRate = preset.learningRate;
      d3.select("#learningRate").property("value", preset.learningRate);

      // Set regularization and update UI
      ...
      // Set regularization rate and update UI
      d3.select("#regularRate")
        .property("value", preset.regularizationRate);
      d3.select("label[for='regularRate'] .value")
        .text(preset.regularizationRate.toString());

        // Select input features and update UI
        for (let key in INPUTS) {
          const enabled = preset.inputFeatures.indexOf(key) !== -1;
          state[key] = enabled;
        
          const canvas = d3.select(`#canvas-${key}`);
          canvas.classed("inactive", !enabled);
          canvas.classed("active", enabled);
        }
  
      // Trigger whole UI update
      parametersChanged = true;
      reset();
      // Add explanation popup to the playground
      showExplanationPopup(preset.explanation);
    });
}
...
function showExplanationPopup(message: string) {
  // Create a dark overlay to blur the playground and bring focus to popup
  ...
  // Create the popup explanation of NN, centered on screen
  ...
  // Create a close button to exit the popup when done reading
  ...
  closeButton.onmouseenter = () => (closeButton.style.color = "#000");
  closeButton.onmouseleave = () => (closeButton.style.color = "#aaa");

  // When close button is clicked, remove popup and overlay to bring playground in focus
  closeButton.onclick = () => {
    popup.remove();
    overlay.remove();
  };

  // Add explanation message of NN and close button to the popup
  popup.appendChild(closeButton);
  const content = document.createElement("div");
  content.innerText = message;
  popup.appendChild(content);

  // Add popup and overlay to the screen
  document.body.appendChild(overlay);
  document.body.appendChild(popup);

  // After 60 seconds close the popup and overlay (if user does not close manually)
  setTimeout(() => {
