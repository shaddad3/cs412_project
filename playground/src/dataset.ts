...
export function classifyMoonsData(numSamples: number, noise: number): Example2D[] {
  const points: Example2D[] = [];
  const crescentPoints = Math.floor(numSamples * 0.4);
  const backgroundPoints = numSamples - crescentPoints;
  ...
  // Place moon in a random position on the canvas
  const dx = randUniform(-3, 3);  // Horizontal offset
  const dy = randUniform(-3, 3);  // Vertical offset

  // Generate positive points in crescent moon shape
  for (let i = 0; i < crescentPoints; i++) {
    const angle = randUniform(angleStart, angleEnd);
    const radius = randUniform(innerRadius, outerRadius);

    let x = radius * Math.cos(angle) + dx;
    let y = radius * Math.sin(angle) + dy;

    // Add little bit of noise to make classification more difficult
    x += randUniform(-1, 1) * noise;
    y += randUniform(-1, 1) * noise;

    points.push({ x, y, label: 1 });
  }

  // Generate negative points across canvas background
  for (let i = 0; i < backgroundPoints; i++) {
    ...
    const shiftedX = x - dx;
    const shiftedY = y - dy;
    const r = Math.sqrt(shiftedX * shiftedX + shiftedY * shiftedY);
    const theta = Math.atan2(shiftedY, shiftedX);

    // Add small delta to allow some padding when determining if negative point is within moon shape 
    const delta = 0.2;

    const inCrescent =
      r >= (innerRadius - delta) &&
      r <= (outerRadius + delta) &&
      theta >= (angleStart - delta) &&
      theta <= (angleEnd + delta);
    
    // Regenerate background point if it falls within moon shape of positive points
    if (inCrescent) {
      i--;
      continue;
    }
...
export function classifyCheckerboardData(numSamples: number, noise: number): Example2D[] {
  const points: Example2D[] = [];

  // Setup 4x4 grid for checkerboard shape
  const canvasExtent = 12;
  const halfExtent = canvasExtent / 2;
  const numSquares = 4;
  const squareSize = canvasExtent / numSquares;

  for (let i = 0; i < numSamples; i++) {
    // For each point, randomly sample across the canvas
    let x = randUniform(-halfExtent, halfExtent);
    let y = randUniform(-halfExtent, halfExtent);

    // Determine row and column in the checker grid
    const col = Math.floor((x + halfExtent) / squareSize);
    const row = Math.floor((y + halfExtent) / squareSize);

    // Alternate between positive and negative class: 
      // even row+col => 1 (positive), odd => -1 (negative)
    const label = (row + col) % 2 === 0 ? 1 : -1;
